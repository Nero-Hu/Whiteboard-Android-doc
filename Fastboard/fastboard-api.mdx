...
- `src`：String。转换后的图片的 URL 地址。你可以从[查询转换任务进度](/api-ref/whiteboard/restful/restful-wb/operations/get-v5-projector-tasks-uuid)响应包体中的 `image` 字段中获取。请确保 App 客户端能够访问该 URL，否则无法正常加载图片。
- `preview`：String。（可选）文档预览图地址。传入后，将在 App 左侧展示预览图。请确保 App 客户端能够访问该 URL，否则无法正常加载预览图。
- `width`：int。图片的宽度（px）。
- `height`：int。图片的长度（px）。
