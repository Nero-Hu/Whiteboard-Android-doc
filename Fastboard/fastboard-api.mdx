...
<h3 className="anchor index-api-fastroomconfig" id="fastroomconfig">FastRoomConfig</h3>
...
该方法可以插入通过[静态文档转换](/doc/whiteboard/android/fastboard-sdk/advanced-features/convert-files#静态文档转换)转换后的静态图片。

**参数**

- `pages`：插入文档的参数设置。详见 <a href="#docpage">DocPage</a>。
- `title`：String。展示文档的子窗口标题。
- `result`：方法调用结果。详见 <a href="#result">FastResult</a>。传入 `FastResult` 实例，SDK 会触发在 `FastResult` 接口中实现的回调，报告方法调用是否成功；传入 `null` 表示不监听回调。
...
- `taskUUID`：String。文档转换任务的 Task UUID，即[发起文档转换任务](/api-ref/whiteboard/restful/restful-wb/operations/post-v5-projector-tasks)请求成功时响应包体中 `uuid` 字段的值。
- `prefixUrl`：String。转换结果文件地址前缀路径，即[查询文档转换进度](/api-ref/whiteboard/restful/restful-wb/operations/get-v5-projector-tasks-uuid)请求成功时响应包体中 `prefix` 字段的值。
- `title`：String。展示文档的子窗口标题。
- `result`：方法调用结果。详见 <a href="#result">FastResult</a>。传入 `FastResult` 实例，SDK 会触发在 `FastResult` 接口中实现的回调，报告方法调用是否成功；传入 `null` 表示不监听回调。
...
- `src`：String。图片或动态 PPT 页的地址。
  - 图片：URL 地址，可以是你自己生成的 URL 地址，也可以是通过[文档转换功能](/doc/whiteboard/android/fastboard-sdk/advanced-features/convert-files)生成的 URL 地址。你可以从[查询转换任务进度](/api-ref/whiteboard/restful/restful-wb/operations/get-v5-projector-tasks-uuid)返回的 `image` 字段中获取该地址。请确保 App 客户端能够访问该 URL，否则无法正常加载图片。
  - 动态 PPT 页：动态 PPT 页：通过[文档转换功能](/doc/whiteboard/android/fastboard-sdk/advanced-features/convert-files)生成的 URI 地址。
- `preview`：String。（可选）图片或动态 PPT 预览图的 URL 地址。动态 PPT 预览图的 URL 地址可以从[查询文档转换进度](/api-ref/whiteboard/restful/restful-wb/operations/get-v5-projector-tasks-uuid)返回的 `preview` 字段获取。传入后，将在 App 左侧展示预览图。请确保 App 客户端能够访问该 URL，否则无法正常加载预览图。
- `width`：int。插入的文档在白板中的宽度（px）。
- `height`：int。插入的文档在白板中的长度（px）。
...
[成功发起文档转换任务](/api-ref/whiteboard/restful/restful-wb/operations/post-v5-projector-tasks)后，可以调用该方法，传入转换后文档的相关参数，SDK 会自动创建子窗口，插入并分页展示转换后的文档。
...
- `logger`：日志记录器实例。你需要传入 `Logger` 接口的实现类，详见 [`Logger`](#logger)；传入 `null` 将使用系统默认的日志记录器。
