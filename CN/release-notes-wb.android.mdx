...
该版本废弃了白板数据中心原有的英国伦敦数据中心（`Region.gb-lon`），并新增了欧洲法兰克福数据中心（`Region.eu`），服务区覆盖欧洲。如需使用该数据中心，请在升级到该版本后更新代码，并在控制台-互动白板服务配置中启用该数据中心。

<VersionTitle icon="/img/icons/version-feature.png">新增特性</VersionTitle>

<ListTitle>PPT 音量控制</ListTitle>

新增 `getSlideVolumeWithCompletionHandler` 和 `updateSlideVolume` 方法用于获取和更新 PPT 内音频的音量。

<ListTitle>PPT URL 拦截替换</ListTitle>

`WhiteSdkConfiguration` 新增 `enableSlideInterrupterAPI` 配置项。开启后，通过调用 `WhiteSdk.setSlideListener(SlideListener)` 实现 PPT 文件 URL 的拦截替换功能。
...
新增 `ProjectorAttributes` 和 `ProjectorOptions`, 对应调用 `manager.addApp` 时传入的参数，详见 [`addApp`](https://github.com/netless-io/window-manager/blob/master/docs/basic.md#open-dynamicstatic-ppt)。

使用新增的 `createSlideApp` 方法即可构建 `WindowAppParam` 对象，优化使用[新版文档转换服务](/doc/whiteboard/android/whiteboard-sdk/advanced-features/convert-files)转换 PPT 文件并插入至白板的流程。
