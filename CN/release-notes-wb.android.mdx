...
在多窗口模式下，SDK 支持以新建窗口的方式插入 PPT 或白板插件等，它们统称为小窗应用。该版本起，`WhiteRoom` 类新增以下方法用于在多窗口模式下管理小窗应用：
...
`RoomParams` 新增 `optimizeOptions` 属性，用于设置白板绘制的性能优化策略。性能优化的方式包括以下两种：
...
- `WhiteWindowParams.fullscreen` 启后，第二次打开 App 没有全屏。
