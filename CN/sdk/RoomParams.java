...
public class RoomParams extends WhiteObject {
     ...
     *
     * @note
     * 为确保传入的 `userPayload` 格式正确，必须为 {@link com.herewhite.sdk.domain.WhiteObject WhiteObject} 的子类。
     *
     ...
     * @param uid 自从 v2.15.0。用户唯一标识符，字符串格式，长度不能超过 1024 字节。如果你使用 2.15.0 及之后版本的 SDK，必须传入该参数。
     ...
     * 初始化房间配置参数并传入用户信息。
     *
     * @deprecated 该方法已经废弃。请使用 {@link RoomParams(String uuid, String roomToken, String uid, Object userPayload) RoomParams}[3/3]。
     *
     * @param uuid       房间 UUID， 即房间唯一标识符。传入的房间 UUID 必须和生成 Room Token 时填入的房间 UUID 一致。
     * @param roomToken  用于鉴权的 Room Token。
     * @param uid 自从 v2.15.0。用户唯一标识符，字符串格式，长度不能超过 1024 字节。如果你使用 2.15.0 及之后版本的 SDK，必须传入该参数。
