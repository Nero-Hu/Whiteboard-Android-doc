...
In multi-window mode, the SDK supports inserting window apps such as PPT or whiteboard plugins as new windows. These are collectively referred to as window apps. Starting from this version, the `Room` object has the following new methods for managing window apps in multi-window mode:
